...
if ($_FILES['plan_Img1']['error'] > 0 || $_FILES['plan_Img2']['error'] > 0 || $_FILES['plan_Img3']['error'] > 0 || $_FILES['plan_Img4']['error'] > 0) {
    ...
    //將照片檔案上傳
    if (!empty($plan_Img1_Name)) {
        move_uploaded_file($filePath_Temp1, $path . $newFileName1);
    }
    if (!empty($plan_Img2_Name)) {
        move_uploaded_file($filePath_Temp2, $path . $newFileName2);
    }
    if (!empty($plan_Img3_Name)) {
        move_uploaded_file($filePath_Temp3, $path . $newFileName3);
    }
    if (!empty($plan_Img4_Name)) {
        move_uploaded_file($filePath_Temp4, $path . $newFileName4);
    }
    ...
    $sql = "UPDATE productSchedule
            ...
            LIMIT 1";
    $statement = $pdo->prepare($sql);
    $statement->bindValue(1, !empty($plan_Img1_Name) ? $newFileName1 : null);
    $statement->bindValue(2, $productId);
    ...
    $statement2 = $pdo->prepare($sql2);
    $params2 = array(
        ':plan_Img2' => !empty($plan_Img2_Name) ? $newFileName2 : null,
        ':productId' => $productId
    );
    ...
    $statement3 = $pdo->prepare($sql3);
    $params3 = array(
        ':plan_Img3' => !empty($plan_Img3_Name) ? $newFileName3 : null,
        ':productId' => $productId
    );
    ...
    $statement4 = $pdo->prepare($sql4);
    $params4 = array(
        ':plan_Img4' => !empty($plan_Img4_Name) ? $newFileName4 : null,
        ':productId' => $productId
    );
